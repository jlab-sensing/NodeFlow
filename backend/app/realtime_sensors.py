...
async def handle_subscribe_sensors(sid, data):
    ...
    return {
        "ok": True,
        "subscribed": [str(sensor_uuid) for sensor_uuid in authorized_uuids],
...
async def handle_unsubscribe_sensors(sid, data):
    try:
        ...
        return {"ok": False, "error": "invalid unsubscription request"}
    ...
    return {
        "ok": True,
        "unsubscribed": [str(sensor_uuid) for sensor_uuid in set(request.sensorUuids)],
    }
