...
async def add_new_logger(logger: LoggerCreate, _current_user: UserTable = Depends(get_current_user)):
    return await create_shared_logger(logger)

@router.get("/{logger_id}", response_model=LoggerRead)
async def get_specific_logger(logger_id: int, _current_user: UserTable = Depends(get_current_user)):
    return await get_shared_logger(logger_id)

@router.put("/{logger_id}", response_model=LoggerRead)
async def update_logger(logger_id: int, update: LoggerUpdate, _current_user: UserTable = Depends(get_current_user)):
    return await update_shared_logger(logger_id, update)

@router.delete("/{logger_id}")
async def delete_logger(logger_id: int, session: Session = Depends(get_session), _current_user: UserTable = Depends(get_current_user)):
