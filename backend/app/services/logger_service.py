...
def normalize_logger(logger: dict) -> dict:
    ...
    if logger_id is None:
        raise HTTPException(
            ...
            detail="Dirtviz returned a logger without an ID",
        )
...
async def get_service_account_email() -> str:
    ...
    if not isinstance(user, dict):
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="Dirtviz returned invalid account data",
        )
    email = str(user.get("email", "")).strip().lower()
    if not email:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="dirtviz api key ownder does not have an email",
        )
    return email
...
async def list_shared_loggers() -> list[dict[str, Any]]:
    ...
    if not isinstance(response, list):
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="Dirtviz returned invalid logger list",
        )

    return [normalize_logger(logger) for logger in response]
...
async def get_shared_logger(logger_id: int) -> dict[str, Any]:
    ...
    logger = next(
        (candidate for candidate in loggers if candidate["logger_id"] == logger_id),
        None,
    )
    if logger is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="logger not found"
        )
    return logger
...
async def create_shared_logger(
    ...
    response = await ents_post(
        ...
        json={
            ...
            "device_eui": (normalize_device_eui(payload.device_eui)),
    ...
    if not isinstance(response, dict):
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="dirtviz returned invalid create response",
        )
    logger_id = response.get("logger_id")
    if logger_id is None:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="dirtviz created logger without returning logger id",
        )

    logger = await ents_get(f"/api/logger/{logger_id}")
...
async def update_shared_logger(logger_id: int, payload: LoggerUpdate) -> dict[str, Any]:
    ...
    updated = await ents_get(f"/api/logger/{logger_id}")
    return normalize_logger(updated)


async def delete_shared_logger(logger_id: int) -> dict[str, Any]:
    await get_shared_logger(logger_id)
    response = await ents_delete(f"/api/logger/{logger_id}")
    return {
        "ok": True,
        "logger_id": logger_id,
        "upstream": response,
