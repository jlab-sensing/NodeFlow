...
function Dashboard() {
  ...
  const { data: groups = [], isLoading, isError } = useUserGroups(axiosPrivate)
    ...
    >
      ...
      >
        ...
        {isLoading ? (
          ...
          <Typography color="error">Unable to load groups.</Typography>
        ) : (
          <Box
            sx={{
              display: 'flex',
          ...
          >
            {groups.map((group) => (
              <GroupBox key={group.uuid} group={group} />
            ))}
          </Box>
        )}
      </Box>
