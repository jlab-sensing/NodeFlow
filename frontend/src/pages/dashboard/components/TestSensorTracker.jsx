...
function TestSensorTracker() {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await axiosPrivate.post('/api/sensor/test/register')
  ...
  const {
    ...
    queryFn: async () => {
      const response = await axiosPrivate.get(`/api/sensor/test/reading`)
  ...
  if (isReadingError) {
    return (
      <Typography color="error">
        {readingError.response?.data?.detail || 'Test sensor unreachable'}
      </Typography>
    )
  }

  const displayedValue =
    typeof data?.value === 'number' ? data.value.toFixed(3) : '--'
  ...
  return (
    ...
    >
      ...
      <Typography color="text.secondary">{data?.unit}</Typography>
