...
function GroupsList() {
  ...
  const handleGroupToggle = async (groupUuid) => {
    ...
    try {
      ...
      const [devicesResponse, activationPrefResponse] = await Promise.all([
        axiosPrivate.get(`/api/groups/${groupUuid}/devices`),
        axiosPrivate.get(`/api/groups/${groupUuid}/activationPref/`),
      ])
      ...
      setError(
        requestError.response?.data?.detail ||
          "Unable to load this group's devices.",
      )
    } finally {
  ...
  if (isError) {
    return <Typography color="error">Error loading groups.</Typography>
  }
        ...
        {!hasGroups ? (
          ...
          groups.map((group) => {
            ...
            const activationSensor = sensors.find(
              (sensor) => sensor.id === activationPreference?.sensor_id,
            )
            ...
            return (
              <Accordion
                key={group.uuid}
                expanded={isExpanded}
                onChange={() => handleGroupToggle(group.uuid)}
                disableGutters
                sx={{
                  borderRadius: '8px',
                  overflow: 'hidden',
              ...
              >
                ...
                >
                  ...
                  >
                    <Button
                      variant="contained"
                      onClick={() => navigate(`/groups/${group.uuid}/edit`)}
                      sx={{
