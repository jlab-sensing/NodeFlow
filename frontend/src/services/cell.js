...
export const updateCell = async (cellId, updatedData, accessToken) => {
  return axios
    .put(
      `${import.meta.env.VITE_API_BASE_URL}/api/cell/${cellId}`,
      updatedData,
      {
        headers: {
...
export const pollCellDataResult = (taskId) => {
  return axios
    .get(`${import.meta.env.VITE_API_BASE_URL}/api/status/${taskId}`)
    .then((res) => {
      return res.data
    })
}
