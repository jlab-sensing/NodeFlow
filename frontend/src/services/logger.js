...
export const getLoggers = () => {
  return axios
    .get(`${import.meta.env.VITE_API_BASE_URL}/api/logger/`)
    .then((res) => res.data)
    .catch((error) => {
...
export const addLogger = (name, type, devEui, joinEui, appKey, description, email, axiosPrivate) => {
  ...
  return axiosPrivate
    .post(`${import.meta.env.VITE_API_BASE_URL}/api/logger/`, payload)
    .then((res) => res.data)
    .catch((error) => {
      console.log(error);
      throw error;
...
export const updateLogger = async (loggerId, updatedData, accessToken) => {
  const url = `${import.meta.env.VITE_API_BASE_URL}/api/logger/${loggerId}`;
  try {
...
export const getUserLoggers = (axiosPrivate) => {
  return axiosPrivate.get(`${import.meta.env.VITE_API_BASE_URL}/api/logger/?user=True`).then((res) => res.data);
};
