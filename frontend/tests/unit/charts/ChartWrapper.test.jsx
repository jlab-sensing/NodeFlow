...
import ChartWrapper from '../../../src/charts/ChartWrapper'
import { getMaxAxisAndStepValues } from '../../../src/charts/alignAxis'
...
describe('testing side button events', () => {
  it('should toggle zoom', async () => {
    ...
    expect(
      ChartJS.getChart(chartElement).config.options.plugins.zoom.zoom.drag
        .enabled,
    ).toBe(false)
  ...
  it('should toggle pan', async () => {
    ...
    expect(
      ChartJS.getChart(chartElement).options.plugins.zoom.pan.enabled,
    ).toBe(true)
  ...
  it('should toggle fullscreen modal', async () => {
    ...
    const fullscreenBtnElement = await screen.findByLabelText(/Fullscreen/i)
    let fullscreenModalElement =
      await screen.queryByTestId(/^fullscreen-modal$/)
